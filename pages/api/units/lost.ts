...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (unit.status === "LOST") return { kind: "already" as const };
      if (unit.status === "SCRAPPED") return { kind: "invalid_state" as const, status: unit.status };
      ...
      if (shouldDecrement) {
        ...
        return {
          kind: "ok" as const,
          unit: { id: unit.id, code, status: "LOST" as const },
    ...
    console.error("POST /api/units/lost error:", error);
    return res.status(500).json({ error: "Failed to mark unit as lost" });
  }
}
