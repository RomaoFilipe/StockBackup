...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (result.kind === "invalid_state") {
      return res.status(400).json({ error: "Unit is not in repair (cannot repair-in)" });
    }

    await logUserAdminAction({
      tenantId: session.tenantId,
      actorUserId: session.id,
      action: "UNIT_REPAIR_IN",
      note: `Unit received from repair: ${code}`,
      payload: {
        code,
        reason: reason ?? null,
        costCenter: costCenter ?? null,
    ...
    logInfo("Unit received from repair", { tenantId: session.tenantId, userId: session.id, code }, req);

    return res.status(200).json(result);
  } catch (error) {
    console.error("POST /api/units/repair-in error:", error);
    return res.status(500).json({ error: "Failed to receive unit from repair" });
  }
}
